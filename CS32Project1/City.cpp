...
void City::moveFlatulans()
{
    ...
    //**********************************************************//
    //NEW WRITTEN CODE TO DETECT DIAGONAL FLATULANS FOR HISTORY //
    //**********************************************************//
    int r = player()->row();
    int c = player()->col();
    for (int i= 0; i < m_nFlatulans; i++) //Checks all flatulans to see if any are diagonal to the player's location
    {
        int fr = m_flatulans[i]->row();
        int fc = m_flatulans[i]->col();
        if (((r-1 == fr) & (c-1 == fc)) || ((r-1 == fr) & (c+1 == fc)) || ((r+1 == fr) & (c-1 == fc)) || ((r+1 == fr) & (c+1 == fc))) //Indicates and compares the four diagonals to the player's location and if a flatulan exists there
        {
            m_history.record(r, c);
            break; //If a flatulan is located, the for loop ends so the record function is not repeated
        }
    }
}
